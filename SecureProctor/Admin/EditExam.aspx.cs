       ...
       */
        ...
        protected void DeleteProviderUploadedFile(string filename)
        {
            try
            {
                ...
                //BProvider objBExamProvider = new BProvider();
                //objBEExamProvider.IntExamID = Convert.ToInt32(Request.QueryString["ExamID"].ToString());
                //objBEExamProvider.strOriginalFileName = filename;



                //System.IO.FileInfo objFileName = new System.IO.FileInfo(Server.MapPath(System.Configuration.ConfigurationManager.AppSettings["ProviderUploadPath"]) + "\\" + filename);
        ...
        private void GetAdditionalInstructionsByExam()
        {
            ...
            else
            {
                if (ViewState["ADDITIONALRULE_OLD"] != null)
                {
                    ...
                    gvAllowed.DataSource = ViewState["ADDITIONALRULE_OLD"] as DataTable;
                    gvAllowed.DataBind();
